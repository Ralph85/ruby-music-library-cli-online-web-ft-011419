class MusicLibraryController
  ...
  def list_artists
    ...
    artist = sorted_library.collect {|object|"#{object.name}"}.uniq
    artist.each {|artist| puts "#{artist.index(artist) + 1}. #{artist}"}
  end
  ...
  def list_songs_by_genre
    ...
    genre_songs.each {|song|puts "#{genre_songs.index(song) + 1}. #{song.artist.name} - #{song.name} - #{song.name}"} unless genre_songs == nil
  end
